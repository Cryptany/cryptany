...
namespace Cryptany.Core.ConfigOM
{
    [Serializable]
    [DbSchema("services")]
    [Table("RulesStatements")]
    public class RuleStatement : EntityBase
	{
		...
		[Relation(RelationType.OneToMany, typeof(Statement), "RuleStatement")]
		public EntityCollection<Statement> Statements
		{
			get
			{
				return GetValue<EntityCollection<Statement>>("Statements");
			}
			set
			{
