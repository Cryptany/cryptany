...
{
    public class HttpMessageManagerTest : HTTPMessageManager
    {
        ...
        public HttpMessageManagerTest(ConnectorSettings cs, ILogger logger)
            : base(cs, logger)
        {
        ...
        public override HTTPMessageParameters DecodeRequest(NameValueCollection queryString)
        {
            HTTPMessageParameters mp = new HTTPMessageParameters();
                ...
                mp.UID = queryString["id"];
                mp.Text = queryString["text"];
                mp.MSISDN = queryString["msisdn"];
